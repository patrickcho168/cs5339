...
testFolderB = "test_stg2/"
testFolderA = "test_stg1/dummy/"
...
def main():
    ...
    # # Save Classifier
    # now = datetime.datetime.now()
    paramsFilename = 'params3_2017-04-13-20-12.pkl'
    ...
    predictions = []
    testIds = []
    for root, _, files in os.walk(testFolderA):
        for file_name in files:
            file_path = os.path.join(root, file_name)
    ...
    for root, _, files in os.walk(testFolderB):
        for file_name in files:
            file_path = os.path.join(root, file_name)
            features = process_image_file(file_path)
            flbase = os.path.basename(file_path)
            testIds.append(file_path)
            prediction = classifier.predict_proba(features)
            
            predictions.append(prediction[0])
    print (predictions)
    print (testIds)
    create_submission(predictions, testIds, "3A")
    # while True:
    #     try:
