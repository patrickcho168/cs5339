...
#Return images from train directory...
def load_train():
    X_train = []
    X_train_id = []
    y_train = []
    ...
    print(X_train, y_train, X_train_id)
...
def read_and_normalize_test_data(num):
    start_time = time.time()
    if num == 1:
        test_data, test_id = load_test()
    else:
        test_data, test_id = load_test2()
...
def run_cross_validation_create_models(nfolds=10):
    ...
    for train_index, test_index in kf:
        ...
        models.append(model)
        model.save('model' + IDENTIFIER + '_' + idx + '.h5')
        idx = idx + 1
...
def run_cross_validation_process_test(info_string, models, num):
    batch_size = 16
    num_fold = 0
    yfull_test = []
    test_id = []
    ...
    for i in range(nfolds):
        ...
        num_fold += 1
        print('Start KFold number {} from {}'.format(num_fold, nfolds))
        test_data, test_id = read_and_normalize_test_data(num)
...
run_cross_validation_process_test(info_string, models,1)    
