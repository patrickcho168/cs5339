...
RETf STR2( float &x, const float y ) { return STR(x, SET(y)); } // UPDATED
...
RETf MUL( const __m128 x, const __m128 y ) { return _mm_mul_ps(x, y); } // UPDATED
RETf MUL2( const __m128 x, const float y ) { return MUL(x, SET(y)); } // UPDATED
RETf MUL3( const float x, const __m128 y ) { return MUL(SET(x), y); } // UPDATED
RETf INC( __m128 &x, const __m128 y ) { return x = ADD(x, y); } // UPDATED
RETf INC2( float &x, const __m128 y ) { __m128 t = ADD(LD(x), y); return STR(x, t); }
RETf DEC( __m128 &x, const __m128 y ) { return x = SUB(x, y); } // UPDATED
RETf DEC2( float &x, const __m128 y ) { __m128 t = SUB(LD(x), y); return STR(x, t); }
RETf MIN2( const __m128 x, const __m128 y ) { return _mm_min_ps(x, y); }
